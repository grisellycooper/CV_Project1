                   ...
                   cv::Mat src)
{
    ...
    for (int i = 0; i < tmpCenters.size(); i++)
    {
        if (cv::norm(avg - tmpCenters[i]) < radialZone + ceil(minDistTwoCenters/2))
              ...
              float &minDistControlPoints)
{
    ...
    {
        //std::cout<<" -- 1: " ;
        float currDist, minDist, maxDist;
        int minIndex, c = 0;
        std::vector<float> distances;
        maxDist = 0.0;          /// MaxDistance of displacement between a previous frame point and the current frame point
        for (int k = 0; k < iPatternSize; k++)
        {
            //std::cout<<"*1 "<<std::endl;
            cv::Point2f tmp = previousPointbuf[k];
            minDist = 1000.0;               /// MinDistance of displacement between a previous frame point and the current frame point
            minDistControlPoints = 1000.0;            /// MinDistance between two control points among all pattern
            minIndex = 0;
            for (int i = 0; i < tmpPointBuf.size(); i++)
            {
                ...
                if (minDistControlPoints > currDist && k != i)
                {
                    minDistControlPoints = currDist;                    
                }
            }
        ...
        if(maxDist < (minDistControlPoints/3))
        {
            //std::cout<<" -- 1: " <<std::endl;                        
            gridFound = true;
        }
...
bool verifyOrder(std::vector<cv::Point2f> &tmpPB, int patternHeigh, int patternWidth)
{
    ...
    if (preSelectedLines.size() == patternHeigh)
    {
        ...
        {
            //get slope 
            //slope = preSelectedLines[i][1]/preSelectedLines[i][0];
